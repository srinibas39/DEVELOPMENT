     ...
     for(let i=0;i<bothInnings.length;i++){
         ...
         for(let j=0;j<tr.length-1;j++){
            ...
            if(td.length>1){ 
                ...
                leaderboards(teamName,Batsman,runs,balls,fours,sixes);
...
function leaderboards(teamName,Batsman,runs,balls,fours,sixes){
    runs=Number(runs);
    balls=Number(balls);
    fours=Number(fours);
    sixes=Number(sixes);
    for(let i=0;i<leaderboards.length;i++){
        let batsmanObject=leaderboards[i];
        if(batsmanObject.teamName==teamName&& batsmanObject.batsman==Batsman){
    ...
    leaderboards.push(batsmanObject);
}